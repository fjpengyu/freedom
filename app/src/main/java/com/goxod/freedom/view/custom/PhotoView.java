...
/**
 * Created by Levey on 2015/12/20
 * Author: bm-x
 * Git: https://github.com/bm-x/PhotoView
 * Add Fresco support on 2016/03/03
 */
public class PhotoView extends ImageView {
    ...
    private void init() {
        ...
        if (mDraweeHolder == null) {
            mDraweeHolder = DraweeHolder.create(FTool.getHierarchy(getContext(),false), getContext());
        }
    }
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private class Transform implements Runnable {
        ...
        void withRotate(int fromDegrees, int toDegrees) {
            mRotateScroller.startScroll(fromDegrees, 0, toDegrees - fromDegrees, 0, ANIM_DURING);
        }
    ...
    public void animaFrom(Info info) {
        ...
        if (isInit) {
            ...
            mAnimMatrix.reset();
            mAnimMatrix.postTranslate(-mBaseRect.left, -mBaseRect.top);
            mAnimMatrix.postTranslate(ocx - mBaseRect.width() / 2, ocy - mBaseRect.height() / 2);
    ...
    public void animaTo(Info info, Runnable completeCallBack) {
        if (isInit) {
            ...
            mTranslate.withRotate((int) mDegrees, (int) info.mDegrees, ANIM_DURING * 2 / 3);
    ...
    public void setImage(final String uri) {
        final ImageRequest request = ImageRequestBuilder.newBuilderWithSource(Uri.parse(uri))
                .setResizeOptions(new ResizeOptions(2000, 2000))
